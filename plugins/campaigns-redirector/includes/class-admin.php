...
class CRMS_Admin {
	...
	private function save_security_settings($post) {
		...
		$opt['block_blank_ua'] = !empty($post['block_blank_ua']);
		...
		if (isset($post['scanner_ua_patterns'])) {
			$opt['scanner_ua_patterns'] = trim((string) wp_unslash($post['scanner_ua_patterns']));
		}

		if (isset($post['automation_ua_patterns'])) {
			$opt['automation_ua_patterns'] = trim((string) wp_unslash($post['automation_ua_patterns']));
		}
			...
			<table class="form-table">
				...
				<tr>
					...
					<td>
						...
						<p class="description">Détecte SafeLinks, Proofpoint, Mimecast, prefetch/prerender, méthodes non-GET, etc.</p>
					</td>
				</tr>
				<tr>
					<th scope="row">User-Agent vide</th>
					<td>
						<label>
							<input type="checkbox" name="block_blank_ua" value="1" <?php checked(!empty($opt['block_blank_ua'])); ?>>
							Bloquer les requêtes sans User-Agent
						</label>
						<p class="description">Recommandé pour les campagnes newsletters. Désactiver seulement si un vrai clic humain est identifié sans User-Agent dans les logs.</p>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						...
						<p class="description">Intervalle minimal entre deux requêtes suspectes depuis la même IP. Les requêtes sans User-Agent, avec headers de prélecture ou avec UA scanner restent bloquées.</p>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="scanner_ua_patterns">UA scanners connus</label></th>
					<td>
						<textarea id="scanner_ua_patterns" name="scanner_ua_patterns" rows="9" class="large-text code"><?php echo esc_textarea($opt['scanner_ua_patterns'] ?? ''); ?></textarea>
						<p class="description">Un motif par ligne, insensible à la casse. Éviter les mots génériques comme <code>apple</code>, <code>google</code>, <code>chrome</code>, <code>safari</code> ou <code>mozilla</code>.</p>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="automation_ua_patterns">UA automatisés</label></th>
					<td>
						<textarea id="automation_ua_patterns" name="automation_ua_patterns" rows="7" class="large-text code"><?php echo esc_textarea($opt['automation_ua_patterns'] ?? ''); ?></textarea>
						<p class="description">Clients techniques à bloquer en 204 : curl, wget, Python, headless browsers, etc.</p>
					</td>
				</tr>
				<tr>
					...
					<tbody>
						<?php foreach ($logs as $row): ?>
							<tr>
								...
								<td><code style="white-space: nowrap;"><?php echo esc_html($row['signals'] ?? ''); ?></code></td>
